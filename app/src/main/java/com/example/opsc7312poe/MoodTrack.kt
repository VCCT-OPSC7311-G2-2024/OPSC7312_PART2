...
class MoodTrack : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        timeFrameSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>, view: View?, position: Int, id: Long) {
                when (parent.getItemAtPosition(position).toString()) {
                    "Last 12 Days" -> getMoodsForTimeFrame(getLast7Days())
                    "Weekly" -> getMoodsForTimeFrame(getLastWeekDays())
                }
            }
        ...
        getMoodsForTimeFrame(getLast7Days())
    }

    private fun getMoodsForTimeFrame(dates: List<String>) {
        ...
        userMoodsRef.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                val moodValues = mutableListOf<Int>() // List to hold mood values for averaging
                ...
                // Calculate average mood for the past 7 days if there are mood values
                val averageMood = if (moodValues.isNotEmpty()) {
                    moodValues.average().toFloat() // Calculate average
                } else {
                    0f // Default if no values
                }

                // Add an entry for the average mood on the right
                moodData.add(BarEntry(moodData.size.toFloat(), averageMood))

                // If we have enough data, add an entry for the previous 7 days
                if (dates.size > 7) {
                    val previousMoodValues = mutableListOf<Int>()
                    val previousDates = getDaysFromNow(7) // Get the previous week
                    previousDates.forEach { date ->
                        val previousMoodValue = dataSnapshot.child(date).child("mood").getValue(String::class.java)
                        previousMoodValue?.let {
                            previousMoodValues.add(Mood.fromString(it).value)
                        }
                    }
                    val averagePreviousMood = if (previousMoodValues.isNotEmpty()) {
                        previousMoodValues.average().toFloat()
                    } else {
                        0f // Default if no previous values
                    }

                    // Add an entry for the average of the previous 7 days
                    moodData.add(BarEntry(moodData.size.toFloat(), averagePreviousMood))
                }

                displayMoodGraph(moodData, dates)
    ...
    private fun displayMoodGraph(moodData: List<BarEntry>, lastDays: List<String>) {
        moodChart.clear()
        ...
        moodChart.xAxis.valueFormatter = if (lastDays.size == 7) {
